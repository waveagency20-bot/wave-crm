  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#080f08', color: 'white' }}>
      ...
      <div className="ml-60 flex-1 flex overflow-hidden" style={{ height: '100vh' }}>
        ...
        {/* Settings nav */}
        <div className="flex-shrink-0 overflow-y-auto py-8 px-4"
          style={{ width: '220px', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
          <p className="text-xs uppercase tracking-wider px-3 mb-3"
            style={{ color: 'rgba(255,255,255,0.3)', letterSpacing: '0.08em' }}>Settings</p>
          {navSections.map(s => (
            <button key={s.id}
          ...
          {activeSection === 'profile' && (
            <div>
              <div className="mb-6">
                ...
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Manage your personal account details</p>
              </div>
              ...
              <Card>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', margin: '4px 0 0' }}>Leave blank to keep current password</p>
                  </div>
                  {passwordError && (
          ...
          {activeSection === 'company' && (
            <div>
              <div className="mb-6">
                ...
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Your business information shown across Wave CRM</p>
              </div>
              <Card>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          ...
          {activeSection === 'team' && (
            <div>
              <div className="mb-6">
                ...
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Manage your team and control access permissions</p>
              </div>
              ...
              {activeTeamTab === 'members' && (
                <div>
                  {isAdmin && (
                    <Card>
                      <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'white', margin: '0 0 16px' }}>Invite a team member</h3>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '12px', alignItems: 'flex-end' }}>
                        <div>
                          <label style={labelStyle}>Email Address</label>
                          <input style={inputStyle} type="email" placeholder="colleague@company.com"
                  ...
                  <Card>
                    <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'white', margin: '0 0 16px' }}>{team.length} members</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      {team.map(member => (
                        <div key={member.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                          ...
                          <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.3)' }}>
                            {member.status === 'invited' ? '📨 Invited' : `Joined ${member.joined}`}
                          </div>
                          {isAdmin ? (
                    ...
                    <div>
                      ...
                      <Card>
                        ...
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                          <button onClick={() => setRolePermissions({ ...rolePermissions, [activePermRole]: Object.fromEntries(allPermissions.map(p => [p.key, true])) as any })}
                          ...
                          <button onClick={() => setRolePermissions({ ...rolePermissions, [activePermRole]: Object.fromEntries(allPermissions.map(p => [p.key, false])) as any })}
                          ...
                          <button onClick={() => setRolePermissions({
                            ...rolePermissions,
                            [activePermRole]: {
                              view_contacts: true, add_contacts: true, delete_contacts: activePermRole === 'Manager',
                              view_pipeline: true, manage_pipeline_stages: false, view_revenue: false,
                              send_campaigns: activePermRole === 'Manager', invite_team: false, manage_billing: false,
                              view_analytics: activePermRole === 'Manager', view_all_leads: activePermRole === 'Manager',
                              view_settings: true,
                            }
                          })}
          ...
          {activeSection === 'billing' && (
            <div>
              <div className="mb-6">
                ...
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Manage your subscription and payment method</p>
              </div>
              ...
              <Card>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <a href="/expired" style={{ padding: '10px 20px', borderRadius: '10px', fontSize: '13px', fontWeight: 600, textDecoration: 'none', background: 'rgba(0,255,136,0.12)', color: '#00ff88', outline: '1px solid rgba(0,255,136,0.25)' }}>
                    Upgrade Plan
                  </a>
                </div>
              </Card>
              ...
              {/* Payment method */}
              <Card>
                <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'white', margin: '0 0 16px' }}>Payment Method</h3>
                <div style={{ display: 'flex', gap: '12px' }}>
                  {[{ icon: '💳', label: 'Card (Stripe)', desc: 'Visa, Mastercard' }, { icon: '📱', label: 'M-Pesa', desc: 'Safaricom Daraja' }].map(method => (
                    <div key={method.label} style={{ flex: 1, padding: '16px', borderRadius: '12px', cursor: 'pointer', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: '12px', transition: 'all 0.15s' }}
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>{method.desc}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeSection === 'notifications' && (
            <div>
              <div className="mb-6">
                ...
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Choose what you want to be notified about</p>
              </div>
              <Card>
                <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'white', margin: '0 0 16px' }}>Notification channels</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {[
                    ...
                    <div key={item.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                      ...
                      <Toggle value={notifications[item.key as keyof typeof notifications] as boolean}
              ...
              <Card>
                <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'white', margin: '0 0 16px' }}>Notify me when</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {[
                    ...
                    { key: 'dealConverted', label: 'A deal is converted', desc: 'Celebrate every win 🎉' },
          ...
          {activeSection === 'integrations' && (
            <div>
              <div className="mb-6">
                ...
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Connect Wave CRM to your communication tools</p>
              </div>

              {[
                { key: 'whatsapp', icon: '💬', name: 'WhatsApp Business API', desc: 'Meta · Send & receive WhatsApp messages', connected: integrations.whatsappConnected, inputLabel: 'WhatsApp Business Number', inputPlaceholder: '+254 7XX XXX XXX', inputValue: integrations.whatsappNumber, onChange: (v: string) => setIntegrations(p => ({ ...p, whatsappNumber: v })), onToggle: () => setIntegrations(p => ({ ...p, whatsappConnected: !p.whatsappConnected })) },
                { key: 'resend', icon: '📧', name: 'Resend', desc: 'Email sending · Transactional & campaigns', connected: integrations.resendConnected, inputLabel: 'Resend API Key', inputPlaceholder: 're_xxxxxxxxxxxxxxxxxxxx', inputValue: integrations.resendKey, onChange: (v: string) => setIntegrations(p => ({ ...p, resendKey: v })), onToggle: () => setIntegrations(p => ({ ...p, resendConnected: !p.resendConnected })) },
                { key: 'at', icon: '📱', name: "Africa's Talking", desc: 'SMS · Safaricom, Airtel, Telkom coverage', connected: integrations.africasTalkingConnected, inputLabel: "Africa's Talking API Key", inputPlaceholder: 'atsk_xxxxxxxxxxxxxxxxxxxx', inputValue: integrations.africasTalkingKey, onChange: (v: string) => setIntegrations(p => ({ ...p, africasTalkingKey: v })), onToggle: () => setIntegrations(p => ({ ...p, africasTalkingConnected: !p.africasTalkingConnected })) },
              ].map(integration => (
                <div key={integration.key} className="rounded-2xl p-6 mb-4"
                  style={{ background: 'rgba(10,20,10,0.8)', border: '0.5px solid rgba(0,255,136,0.1)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div>
                      ...
                      <input style={inputStyle} placeholder={integration.inputPlaceholder} value={integration.inputValue}
                        onChange={e => integration.onChange(e.target.value)}
                        onFocus={e => e.target.style.borderColor = 'rgba(0,255,136,0.4)'}
                        onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
                    </div>
                    <button onClick={integration.onToggle}
              ...
              <div className="rounded-2xl p-6 mb-4" style={{ background: 'rgba(10,20,10,0.8)', border: '0.5px solid rgba(0,255,136,0.1)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
