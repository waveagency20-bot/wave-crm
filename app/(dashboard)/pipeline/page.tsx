...
const validateEmail = (email: string) => !email || /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)
...
export default function PipelinePage() {
  ...
  // Mobile: which stage is currently selected in the list view
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'leads' }, (payload) => {
        ...
        setStages(prev => {
          const withoutLead = prev.map(stage => ({ ...stage, leads: stage.leads.filter(l => l.id !== updatedLead.id) }))
          return withoutLead.map(stage => {
            if (stage.id !== updatedLead.stage_id) return stage
            const existingLead = prev.flatMap(s => s.leads).find(l => l.id === updatedLead.id)
            if (!existingLead) return stage
            return { ...stage, leads: [...stage.leads, { ...existingLead, stageId: updatedLead.stage_id, daysInStage: updatedLead.days_in_stage || 0 }] }
          })
        })
      })
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'leads' }, async (payload) => {
        ...
        const { data: contact } = await supabase
          .from('contacts').select('name, company').eq('id', newLead.contact_id).single()

        const lead: Lead = {
          id: newLead.id, name: contact?.name || 'Unknown', company: contact?.company || '',
          value: newLead.value || 0, source: newLead.source || 'manual',
          assigned: newLead.assigned_to || '', assignedFull: newLead.assigned_to || '',
          tags: newLead.tags || [], daysInStage: newLead.days_in_stage || 0,
          hot: newLead.hot || false, stageId: newLead.stage_id,
        }

        setStages(prev => prev.map(stage =>
          stage.id === newLead.stage_id ? { ...stage, leads: [lead, ...stage.leads] } : stage
        ))
      })
      .subscribe()
  ...
  const initPipeline = async () => {
    ...
    try {
      ...
      const { data: existingStages } = await supabase
        ...
        .eq('organisation_id', userProfile.organisation_id).order('position')
      ...
      if (stageList.length === 0) {
        const { data: newStages } = await supabase
          ...
          .insert(defaultStages.map((s, i) => ({ organisation_id: userProfile.organisation_id, label: s.label, color: s.color, position: i })))
          .select()
        stageList = newStages || []
      }

      const { data: leads } = await supabase
        .from('leads').select('*, contacts(name, company)')
      ...
      const mappedStages: Stage[] = stageList.map(stage => ({
        ...
        leads: (leads || []).filter((l: any) => l.stage_id === stage.id).map((l: any) => ({
          id: l.id, name: l.contacts?.name || 'Unknown', company: l.contacts?.company || '',
          value: l.value || 0, source: l.source || 'manual',
          assigned: l.assigned_to || '', assignedFull: l.assigned_to || '',
          tags: l.tags || [], daysInStage: l.days_in_stage || 0,
          hot: l.hot || false, stageId: stage.id,
        })),
      }))

      setStages(mappedStages)
      if (mappedStages.length > 0) setForm(prev => ({ ...prev, stage: mappedStages[0].id }))
    } catch (err) {
  ...
  // ---------------------------------------------------------------------------
  const handleDragStart = (lead: Lead, stageId: string) => setDragging({ lead, fromStage: stageId })

  const handleDrop = async (toStageId: string) => {
    if (!dragging || dragging.fromStage === toStageId) { setDragging(null); setDragOver(null); return }
    ...
    setStages(prev => prev.map(stage => {
      if (stage.id === fromStageId) return { ...stage, leads: stage.leads.filter(l => l.id !== draggedLead.id) }
      if (stage.id === toStageId)   return { ...stage, leads: [...stage.leads, { ...draggedLead, daysInStage: 0, stageId: toStageId }] }
      return stage
    }))

    try {
      await supabase.from('leads').update({
        stage_id: toStageId, days_in_stage: 0,
  ...
  // Mobile: move a lead to a different stage
  const handleMobileMove = async (lead: Lead, toStageId: string) => {
    ...
    setStages(prev => prev.map(stage => {
      ...
      if (stage.id === toStageId)   return { ...stage, leads: [...stage.leads, { ...lead, daysInStage: 0, stageId: toStageId }] }
      return stage
    }))
    ...
    try {
      ...
      if (form.email) {
        const { data: existingContact } = await supabase
          .from('contacts').select('id').eq('organisation_id', orgId).eq('email', form.email).single()
      ...
      if (!contactId) {
        const { data: newContact } = await supabase
          .from('contacts')
          .insert({ organisation_id: orgId, name: form.name, company: form.company, email: form.email, phone: form.phone, source: form.source, assigned_to: form.assigned, last_contact: new Date().toISOString() })
          .select().single()
      ...
      const { error } = await supabase.from('leads').insert({
        organisation_id: orgId, contact_id: contactId, stage_id: stages[0]?.id,
        value: parseInt(form.value.replace(/[^0-9]/g, '')) || 0,
        source: form.source, assigned_to: assignee?.full || form.assigned,
        tags: form.tags ? form.tags.split(',').map(t => t.trim()).filter(Boolean) : [],
        days_in_stage: 0, hot: false,
      })
  ...
  const handleDeleteStage = async (stageId: string) => {
    ...
    if (stage && stage.leads.length > 0) { alert(`Cannot delete "${stage.label}" — move its ${stage.leads.length} lead(s) first.`); return }
  ...
  const handleAddStage = async () => {
    const { data: newStage } = await supabase
      .from('pipeline_stages')
      .insert({ organisation_id: orgId, label: 'New Stage', color: '#a78bfa', position: stages.length })
      .select().single()

    if (newStage) setStages(prev => [...prev, { id: newStage.id, label: newStage.label, color: newStage.color, leads: [] }])
  }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', background: '#060d06' }}>
        <Sidebar />
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: isMobile ? 0 : '240px' }}>
          <span style={{ fontSize: '14px', color: 'rgba(255,255,255,0.3)' }}>Loading pipeline...</span>
        </div>
      </div>
    )
  }
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#060d06', color: 'white', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: '#060d06',
          position: 'sticky', top: isMobile ? '56px' : 0, zIndex: 50,
        }}>
          <div>
            <h1 style={{ fontSize: isMobile ? '18px' : '20px', fontWeight: 700, color: 'white', margin: 0 }}>Sales Pipeline</h1>
            <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', margin: '3px 0 0' }}>
              {allLeads.length} leads · KES {totalValue.toLocaleString()}
            </p>
          </div>
          <div style={{ display: 'flex', gap: '8px', width: isMobile ? '100%' : 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '6px 10px', borderRadius: '8px', background: 'rgba(0,255,136,0.08)', border: '1px solid rgba(0,255,136,0.2)' }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#00ff88', boxShadow: '0 0 6px #00ff88' }} />
              <span style={{ fontSize: '11px', color: '#00ff88', fontWeight: 600 }}>Live</span>
            </div>
            {!isMobile && (
              <button onClick={() => setManageOpen(true)}
                style={{ padding: '8px 14px', borderRadius: '10px', fontSize: '12px', fontWeight: 500, cursor: 'pointer', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.5)' }}>
                Manage Stages
              </button>
            )}
            <button
              onClick={() => { setForm({ ...emptyForm, stage: stages[0]?.id || '' }); setErrors({}); setDrawerOpen(true) }}
              style={{ flex: isMobile ? 1 : 'none', padding: '8px 16px', borderRadius: '10px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', background: 'rgba(0,255,136,0.12)', border: '1px solid rgba(0,255,136,0.25)', color: '#00ff88' }}>
              + Add Lead
            </button>
          </div>
        </div>
        ...
        {/* ── Stage summary bar ── */}
        <div style={{ padding: isMobile ? '10px 16px' : '12px 32px', display: 'flex', gap: isMobile ? '12px' : '24px', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.04)', background: '#070e07', overflowX: 'auto', flexWrap: isMobile ? 'nowrap' : 'wrap' }}>
          {stages.map((stage, i) => (
            <div
              key={stage.id}
              onClick={() => isMobile && setMobileStageIndex(i)}
              style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: isMobile ? 'pointer' : 'default', flexShrink: 0, padding: isMobile ? '4px 8px' : '0', borderRadius: isMobile ? '8px' : '0', background: isMobile && mobileStageIndex === i ? `${stage.color}15` : 'transparent', border: isMobile && mobileStageIndex === i ? `1px solid ${stage.color}40` : '1px solid transparent' }}
            >
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: stage.color, boxShadow: `0 0 5px ${stage.color}` }} />
              <span style={{ fontSize: '12px', color: mobileStageIndex === i && isMobile ? stage.color : 'rgba(255,255,255,0.5)', fontWeight: mobileStageIndex === i && isMobile ? 600 : 400 }}>{stage.label}</span>
              <span style={{ fontSize: '11px', fontWeight: 700, padding: '1px 6px', borderRadius: '20px', background: `${stage.color}18`, color: stage.color }}>{stage.leads.length}</span>
            </div>
          ))}
        </div>

        {/* ── MOBILE: list view for selected stage ── */}
        {isMobile && (
              ...
              <>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ width: 10, height: 10, borderRadius: '50%', background: stages[mobileStageIndex].color }} />
                    <span style={{ fontSize: '14px', fontWeight: 700, color: 'white' }}>{stages[mobileStageIndex].label}</span>
                    <span style={{ fontSize: '12px', color: stages[mobileStageIndex].color }}>({stages[mobileStageIndex].leads.length})</span>
                  </div>
                  <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)' }}>
                    KES {stages[mobileStageIndex].leads.reduce((s, l) => s + l.value, 0).toLocaleString()}
                  </span>
                </div>

                {stages[mobileStageIndex].leads.length === 0 ? (
                  <div style={{ padding: '48px 24px', textAlign: 'center', borderRadius: '14px', border: `2px dashed ${stages[mobileStageIndex].color}25` }}>
                    <div style={{ fontSize: '28px', opacity: 0.2, marginBottom: '8px' }}>⊕</div>
                    <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.3)' }}>No leads in this stage</div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {stages[mobileStageIndex].leads.map(lead => (
                      <div key={lead.id} style={{ background: '#0a120a', borderRadius: '14px', padding: '16px', border: '1px solid rgba(255,255,255,0.07)' }}>
                        {/* Lead header */}
                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <div style={{ width: 38, height: 38, borderRadius: '12px', flexShrink: 0, background: `${stages[mobileStageIndex].color}15`, color: stages[mobileStageIndex].color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 700 }}>
                              {lead.name.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase()}
                            </div>
                            <div>
                              <div style={{ fontSize: '14px', fontWeight: 700, color: 'white' }}>{lead.name} {lead.hot && '🔥'}</div>
                              <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>{lead.company}</div>
                            </div>
                          </div>
                          <div style={{ fontSize: '15px', fontWeight: 700, color: stages[mobileStageIndex].color }}>
                        ...
                        {lead.tags.length > 0 && (
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', marginBottom: '12px' }}>
                            {lead.tags.map(tag => (
                              <span key={tag} style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '6px', background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.45)' }}>{tag}</span>
                            ))}
                          </div>
                        )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>{lead.assignedFull || lead.assigned}</span>
                            <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.2)' }}>· {lead.daysInStage === 0 ? 'Today' : `${lead.daysInStage}d`}</span>
                          </div>
                          {/* Move to stage dropdown */}
                          <select
                            value={lead.stageId}
                            onChange={e => handleMobileMove(lead, e.target.value)}
                            style={{ fontSize: '11px', padding: '5px 8px', borderRadius: '8px', background: `${stages[mobileStageIndex].color}15`, border: `1px solid ${stages[mobileStageIndex].color}30`, color: stages[mobileStageIndex].color, cursor: 'pointer', outline: 'none' }}
                          >
                            {stages.map(s => (
                              <option key={s.id} value={s.id} style={{ background: '#0a120a', color: 'white' }}>{s.label}</option>
                            ))}
                          </select>
                        </div>
                      </div>
                  ...
                  <div style={{ padding: '18px 20px 14px', borderBottom: `1px solid ${stage.color}18`, position: 'sticky', top: 0, background: '#0a120a', borderRadius: '16px 16px 0 0', zIndex: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <button onClick={() => { setForm({ ...emptyForm, stage: stage.id }); setErrors({}); setDrawerOpen(true) }}
                  ...
                  <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {stage.leads.map(lead => (
                      <div key={lead.id}
                        ...
                        style={{ background: '#111b11', borderRadius: '14px', padding: '18px', border: '1px solid rgba(255,255,255,0.07)', opacity: dragging?.lead.id === lead.id ? 0.3 : 1, cursor: 'grab', transition: 'all 0.15s', boxShadow: '0 2px 12px rgba(0,0,0,0.3)' }}
                        onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.border = `1px solid ${stage.color}45`; el.style.transform = 'translateY(-3px)' }}
                        onMouseLeave={e => { const el = e.currentTarget as HTMLDivElement; el.style.border = '1px solid rgba(255,255,255,0.07)'; el.style.transform = 'translateY(0)' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: '10px', marginBottom: '14px', background: `${stage.color}0e`, border: `1px solid ${stage.color}20` }}>
                          ...
                          <span style={{ fontSize: '15px', fontWeight: 700, color: stage.color }}>KES {lead.value.toLocaleString()}</span>
                        </div>

                        {lead.tags.length > 0 && (
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '14px' }}>
                            {lead.tags.map(tag => (
                              <span key={tag} style={{ fontSize: '11px', padding: '4px 10px', borderRadius: '8px', background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.45)', border: '1px solid rgba(255,255,255,0.08)' }}>{tag}</span>
                            ))}
                          </div>
                        )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '4px 8px', borderRadius: '8px', background: 'rgba(255,255,255,0.04)' }}>
                            ...
                            <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)' }}>{lead.daysInStage === 0 ? 'Today' : `${lead.daysInStage}d`}</span>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    {stage.leads.length === 0 && (
                      <div style={{ padding: '48px 20px', borderRadius: '14px', textAlign: 'center', border: `2px dashed ${dragOver === stage.id ? stage.color + '50' : stage.color + '18'}`, background: dragOver === stage.id ? `${stage.color}05` : 'transparent', transition: 'all 0.2s' }}>
                        <div style={{ fontSize: '28px', opacity: 0.2, marginBottom: '8px' }}>⊕</div>
      ...
      {/* ── Add Lead Drawer — full screen on mobile ── */}
      <div style={{
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid rgba(0,255,136,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h2 style={{ fontSize: '17px', fontWeight: 700, color: 'white', margin: 0 }}>Add New Lead</h2>
            <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', margin: '4px 0 0' }}>All new leads start at the first stage</p>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {sourceOptions.map(s => (
                <button key={s} onClick={() => setForm({ ...form, source: s })}
                  style={{ padding: '7px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: 500, cursor: 'pointer', background: form.source === s ? 'rgba(0,255,136,0.15)' : 'rgba(255,255,255,0.04)', border: form.source === s ? '1px solid rgba(0,255,136,0.35)' : '1px solid rgba(255,255,255,0.08)', color: form.source === s ? '#00ff88' : 'rgba(255,255,255,0.45)' }}>
                  {sourceIcons[s]} {s}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{ padding: '12px 16px', borderRadius: '10px', fontSize: '13px', fontWeight: 600, background: `${stages[0]?.color || '#818cf8'}12`, border: `1px solid ${stages[0]?.color || '#818cf8'}30`, color: stages[0]?.color || '#818cf8', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              {stages[0]?.label || 'Client Contacted'}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {assigneeOptions.map(a => (
                <button key={a.id} onClick={() => setForm({ ...form, assigned: a.id })}
                  ...
                  <div style={{ width: 26, height: 26, borderRadius: '50%', background: `${a.color}20`, color: a.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700 }}>{a.id}</div>
                  {a.full}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input style={inputStyle} placeholder="Retail, Hot Lead" value={form.tags}
      ...
      {/* ── Manage Stages Modal — full screen on mobile ── */}
      <div style={{
        position: 'fixed',
        top:    isMobile ? 0        : '50%',
        left:   isMobile ? 0        : '50%',
        ...
        transform: isMobile ? 'none' : manageOpen ? 'translate(-50%, -50%) scale(1)' : 'translate(-50%, -50%) scale(0.95)',
        ...
        pointerEvents: manageOpen ? 'all' : 'none',
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid rgba(0,255,136,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h2 style={{ fontSize: '17px', fontWeight: 700, color: 'white', margin: 0 }}>Manage Stages</h2>
            <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', margin: '4px 0 0' }}>Rename, recolor or delete pipeline stages</p>
          </div>
          <button onClick={() => setManageOpen(false)} style={{ width: 36, height: 36, borderRadius: '10px', border: 'none', background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
        </div>

        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {stages.map(stage => (
            <div key={stage.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 14px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <div style={{ position: 'relative' }}>
                <div onClick={() => setEditingStage(editingStage?.id === stage.id ? null : stage)}
                  ...
                  <div style={{ position: 'absolute', top: '32px', left: 0, zIndex: 10, background: '#0f1f0f', border: '1px solid rgba(0,255,136,0.2)', borderRadius: '12px', padding: '12px', display: 'flex', flexWrap: 'wrap', gap: '8px', width: '160px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
                    {colorOptions.map(c => (
                      <div key={c} onClick={() => handleColorStage(stage.id, c)}
                        style={{ width: 24, height: 24, borderRadius: '50%', background: c, cursor: 'pointer', border: stage.color === c ? '2px solid white' : '2px solid transparent' }} />
                    ))}
                  </div>
                )}
              </div>
              <input value={stage.label} onChange={e => handleRenameStage(stage.id, e.target.value)}
              ...
              <span style={{ fontSize: '11px', fontWeight: 700, padding: '2px 8px', borderRadius: '20px', background: `${stage.color}18`, color: stage.color }}>{stage.leads.length}</span>
        ...
        <div style={{ padding: '14px 24px 20px', borderTop: '1px solid rgba(0,255,136,0.08)', display: 'flex', gap: '10px' }}>
          <button onClick={handleAddStage} style={{ flex: 1, padding: '11px', borderRadius: '10px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', background: 'rgba(0,255,136,0.08)', border: '1px solid rgba(0,255,136,0.2)', color: '#00ff88' }}>+ Add Stage</button>
